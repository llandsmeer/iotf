...
def lif_timeit(ncells):
    ...
    lif40 = lif_make_timestep40(ncells, 0)
    burn_in = 50
    args = (
...
def hh_timeit(ncells):
    ...
    hh40 = hh_make_timestep40(ncells, 0)
    burn_in = 50
    vs = []
    args = dict(
...
def io_timeit(ncells, compile_full=False):
    ...
    timestep40 = ioperf.model.make_tf_function_40(ngj=0, argconfig=argconfig)
    if compile_full:
    ...
    for i in range(burn_in):
        state = timestep40(state, I_app=I_app, g_CaL=g_CaL)['state_next']
    ms = 1000
    a = time.perf_counter()
    for i in range(ms):
        state = timestep40(state, I_app=I_app, g_CaL=g_CaL)['state_next']
...
def lif_timeit_groq(ncells):
    ...
    print('PATH', path)
    lif = lif_make_timestep40(ncells, 0, compile=False, repeat=True)
...
def hh_timeit_groq(ncells):
    ...
    path = tempfile.mktemp() + '.onnx'
    print('PATH', path)
    hh40 = hh_make_timestep40(ncells, 0, compile=False, repeat=True)
    onnx_model, _ = tf2onnx.convert.from_function(
            function=hh40,
...
def io_timeit_groq(ncells):
    ...
    import groq.runtime
    argconfig = dict( I_app='VARY', g_CaL='VARY' )
    io40 = ioperf.model.make_tf_function_40(ncells=ncells, ngj=0, argconfig=argconfig)
    state = ioperf.model.make_initial_neuron_state(ncells, V_soma=None).numpy()
    path = tempfile.mktemp() + '.onnx'
    print('PATH', path)
    onnx_model, _ = tf2onnx.convert.from_function(
            function=io40,
            input_signature=io40.argspecv2,
            output_path=path,
            opset=16,
            )
    subprocess.call(['groq-compiler', f'-save-stats={path}.stats', '--large-program', '-o', f'{path}', path])
    ...
    for i in range(ms):
        state = program(state=state, i_app=I_app, g_cal=g_CaL)['state_next']
    b = time.perf_counter()
    elapsed = b - a
    print('>'*10, 'seconds/second', elapsed)
    return elapsed
